...
# Global values -----------------------------------------------------------
df_data1 <- datatable(data.frame(col_String = c("A","B","C"),
...
server <- function(input, output) {
  output$vrb_out1a <- renderText("df_data1 <- datatable(data.frame(col_String = c(\"A\",\"B\",\"C\"),
                                 col_Numeric = c(1000, 1/3, 1000 + 1/3),
                                 col_Integer = as.integer(1001:1003),
                                 col_Percent = percent(c(1, 1.25, .33)),
                                 stringsAsFactors = FALSE))")
  
  output$vrb_out1b <- renderText("renderDataTable(df_data1) > dataTtableOutput(\"tbl_out1\")")
  output$vrb_out1c <- renderPrint({str(df_data1)})
  
  output$tbl_out1 <- renderDataTable(df_data1)
